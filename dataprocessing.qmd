...
-   Así, cuando vemos devuelto las respuestas `[1] 23654` y `[1] 44099` quiere decir que para `start_v` y `end_v` está asignado el primer valor donde aparece dichas expresiones , localizadas por primera vez en la posición 23654 y 44099 de nuestro texto.
...
Al iniciar el código el valor que os ha tenido que recuperar, además de almacenar el objeto en la pestaña `Environment` de RStudio, es:
...
-   `paste()` para unir todas las líneas del texto en una sola por entrada y separarlas por un espacio.

-   `if`dentro de `sapply()`define los límites de inicio y fin de cada año y con el parámetro `simplify = TRUE` nos devolverá un vector de cadenas de texto por entrada.

-   `list()` crea la lista que almacena todas las entradas.

#### PASO 5: Asignación de nombres a entradas

De cara al análisis de datos global, deberemos nombrar cada una de las entradas para que luego seamos capaces de identificarlas.
...
-   Hemos aplicado la función `names()`a cada entrada del diario dentro de cada año. Esta función asigna un nombre a cada elemento dentro de un objeto. En una tabla de *Excel* sería lo equivalente a ponerle un nombre a cada columna de nuestro set de datos.

-   Para asignar los nombres de manera automática y que estos sean únicos empleamos la función `paste0()` que concatena diferentes secuencias de texto, en nuestro caso concatenamos cuatro elementos:

    -   Un texto: `"entry_"`.

    -   El índice del año `i`.

    -   El índice de la entrada, en este caso lo determinamos con la función `seq_along()` que va contando la posición de cada uno de los elementos `entries` dentro de la lista `year`.
...
1.  Convertimos el texto en minúsculas y eliminaos los signos de puntuación
